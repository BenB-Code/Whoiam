...
const { selectIds, selectEntities, selectAll, selectTotal } = adapter.getSelectors(selectWindowState);
...
export const selectActiveWindow = createSelector(
  selectAllWindows,
  windows => windows.find(window => window.isActive) || null
);

export const selectOpenWindows = createSelector(selectAllWindows, windows =>
  windows.filter(window => window.status === OPEN)
);

export const selectMaxZIndex = createSelector(selectAllWindows, windows => {
